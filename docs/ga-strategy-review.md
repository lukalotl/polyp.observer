...
**Status (9 Sep 2026).** The following recommendations are now implemented under the semantics fixed in [ga-primitives-contract.md](./ga-primitives-contract.md): distinct elitism, heavy-tailed mutation, epsilon-lexicase selection, the expected-changes-per-child readout with its change-count distribution, truthful effective-world counts (cross/point report one deterministic world), a fresh recorded search seed for every draft with one-click exact replay, independent repeats, the stall pause (modulo rule, paused not completed), the search-health diagnostics (generations since improvement, head convergence among the top ranks, best copies, unique evaluations and repeat share) and the four-section creator with its persistent Experiment summary. The new-draft numbers (population 64, 2 distinct elites, tournament of 2, beta 1.5, uniform crossover at 70%, 5% immigrants, no stall pause) remain hypotheses: they are the trial settings from the table below, not measured winners, and stand until the benchmark plan in the validation section has compared them against the legacy operators, which stay available for every saved run.
