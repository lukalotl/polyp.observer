...
One separate VM worker runs the shared streaming trajectory once over the entire configured horizon, retaining only bounded preview planes. It never allocates the entire spacetime volume. Returned population/fitness-related simulation metrics retain that full horizon. Render layers are uniformly strided to at most 128, at most 8 MiB of spatial planes, and at most 180,000 **occupied** voxels, retaining the first/last complete spatial planes. No spatial cells are silently dropped. `layerTimes` gives each original CA time index; `stride` identifies sampling and `totalSteps` includes the initial layer at t=0. The final interval may be shorter. If even the two complete boundary planes cannot fit, an explicit 422 preview-limit error is returned. This can occur for large dense grids; scientific scoring is unaffected. Runs support odd grids up to 1025 and horizons up to 65,536, under a 1,073,741,824 cell-timestep work budget. Preview uses a bounded 8-request queue, 16-entry/16-MiB cache and 120-second timeout (proxy requests allow an additional five seconds). It cannot alter scoring, run state, RNG or workers. An observer's navigation does not change the scientific experiment.
