...
- Default global limits on a 16-CPU VM are **13 evaluation workers**, **3 active runs**, and **15 total training threads** (coordinators + evaluators). Smaller machines are bounded by `os.availableParallelism()`; an 8-CPU VM still allows six evaluators and seven training threads. `POLYP_MAX_EVALUATION_WORKERS` (1–13), `POLYP_MAX_ACTIVE_RUNS` (1–3), and `POLYP_CPU_BUDGET` may lower these limits. CPU budget also counts coordinators; for example three 2-evaluator runs cannot fit a 7-thread budget. One independently bounded preview worker uses the remaining reservation. Pools are created only after admission. Public production uses one active run, thirteen evaluators and a fourteen-thread training budget.
- Queued runs are strictly FIFO: a smaller request never jumps a resource-blocked head. Admission reserves the run's maximum configured memory growth as well as its worker capacity. If the memory budget is full, the head waits for capacity to be released. Queue metadata is refreshed when capacity changes. `Capacity.allocatedWorkers` counts evaluators; run `workerCount` and health `workers` count actual coordinator/evaluator threads (health also includes preview). Paused runs have no pool.
...
- **Stall pause.** `config.stallGenerations` (0 or omitted = never) applies only to continuous runs, never to `step`. After a generation commits, if `generationsSinceImprovement` is a positive whole multiple of `stallGenerations`, the run pauses through the ordinary pause path: the triggering generation is committed and persisted, the coordinator and evaluators terminate, status is `"paused"` (never `"completed"`), `stopReason` is exactly `Stalled: N generations without improvement.`, and persisted intent is cleared so a restart does not resume it. `start` resumes the same population and clears the reason; because the rule fires at multiples, a resumed run continues to the next multiple instead of re-pausing at once. A generation limit reached on the same generation wins and completes the run.

## History deltas and summary mirrors

Every committed generation appends one `HistoryPoint` carrying the engine's `GenerationMetrics` (including `distinctElites`, `bestCopies`, `generationsSinceImprovement`) plus two manager-derived deltas versus the state before the advance (generation 0 uses a 0/0 baseline):

- `generationEvaluations` = `(state.evaluations − previous.evaluations) / fixtureCount`, where `fixtureCount = trainingSeeds.length + validationSeeds.length`; unique genomes actually scored this generation. Summing it over a complete history and multiplying by `fixtureCount` reproduces `state.evaluations`.
- `generationRepeats` = `state.cacheHits − previous.cacheHits`; candidates that repeated an already-known genome. `generationEvaluations + generationRepeats` equals the candidates bred (`populationSize` at generation 0, `populationSize − eliteCount` afterwards).

`RunSummary.generationsSinceImprovement` and `RunSummary.distinctElites` mirror the latest snapshot metrics wherever a summary is derived from state (load from disk, each generation, import, fork) and are absent when the run has no population. Both are also served by `/api/runs` and the WebSocket `runs` list. `metrics.csv` appends `distinctElites,bestCopies,generationsSinceImprovement,generationEvaluations,generationRepeats` columns; rows retained from older servers leave them blank.
...
Checkpoints are persisted at the configured interval (2–300 seconds), explicit checkpoint, pause/archive, generation limit, stall pause, and SIGINT/SIGTERM shutdown. Clean shutdown preserves running/queued intent. On restart, only persisted running/queued/starting runs with `resumeOnRestart: true` are requeued; paused/completed/failed/archived runs stay stopped. Single-step intent is persisted separately and interrupted steps are not automatically replayed after shutdown or a crash. A corrupt primary falls back to its validated backup. Corrupt primary **and** backup isolate that run instead of crashing other runs; health `recoveryErrors` reports what was recovered/skipped. Schema, model version, state/fitness/lineage/counter invariants and archived populations are validated before use. Validation establishes structural consistency, not proof that an externally supplied score was honestly computed.

Checkpoint validation is tolerant of age, strict about content. History points and archived snapshot metrics written before `distinctElites`, `bestCopies` and `generationsSinceImprovement` existed, or before `generationEvaluations`/`generationRepeats` were recorded, still import and load; the retained history is kept as written (never backfilled), while archived snapshot metrics and the current snapshot are recomputed from the population and therefore gain the keys. Whenever one of those keys **is** present it must be a non-negative safe integer, and the three metric keys must agree exactly with `generationSnapshot(state)` like every other current metric. `storageVersion` stays 1; stored summaries with or without the mirror fields load, and the mirrors are recomputed from state rather than trusted from disk.
...
- At most **64 stored runs**, including archived runs; 64 MiB per checkpoint file and 1 GiB total checkpoint/backup disk budget. Conservative memory reservations cap a run's maximum growth at 64 MiB and the manager at 512 MiB; oversized population × archive combinations are rejected up front. Stopped and queued runs are charged for their retained population, snapshots, cache, history and improvements, plus metadata, rather than unused future retention. Active runs reserve their full configured growth until their workers terminate. Restart uses the same retained-data accounting before readmitting resumable jobs. Health exposes `memory.reservedBytes` and `memory.limitBytes`; these are conservative research-data reservations, not process RSS or a whole-process heap limit. To reclaim storage, export, stop the server, and remove an old run's primary and backup together. There is no destructive delete endpoint.
