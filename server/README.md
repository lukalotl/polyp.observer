...
One separate VM worker runs legacy `simulate()` exactly once over the entire configured horizon. Returned population/fitness-related simulation metrics retain that full horizon. Render layers are uniformly strided to at most 128 and at most 180,000 **occupied** voxels, retaining the first/last complete spatial planes. No spatial cells are silently dropped. `layerTimes` gives each original CA time index; `stride` identifies sampling and `totalSteps` includes the initial layer at t=0. The final interval may be shorter. If even the two complete boundary planes cannot fit, an explicit 422 preview-limit error is returned. Current size bound makes that unusual (129² sites per plane). Preview uses a bounded 8-request queue, 16-entry/16-MiB cache and 30-second timeout. It cannot alter scoring, run state, RNG or workers. An observer's navigation does not change the scientific experiment.
